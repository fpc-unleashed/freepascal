...
```pas
union bitsize 20
  raw: array[0..2] of byte;        { 24 bity = 3 bajty (OK, mieści sie w 3 bajt storage) }
  bitpacked record
    a, b, c, d, e, f, g, h: boolean;
    i, j, k, l, m, n, o, p: boolean;
    q, r, s, t: boolean;            { 20 bitow OK }
  end;
end;
...
union bitsize 8
  BitField: Byte;
  bitpacked record
    a, b, c, d, e, f, g, h, i: boolean;   { 9 bitow > 8 -> COMPILE ERROR }
  end;
end;
```
...
```pas
union of Byte bitsize 8              { default = Byte, max 8 bitow }
  BitField: Byte;
  bitpacked record
    a, b, c: 1;
    pad 5;
...
Spojnosc z C++17 `alignof(T)`, C11 `_Alignof(T)`, Rust `std::mem::align_of::<T>()` - typowe nazewnictwo i semantyka.
...
`pad` is a contextual keyword - lapane only in the bitpacked record body when the next token is not `:` or `,`. Records that name a field `pad` keep working:
