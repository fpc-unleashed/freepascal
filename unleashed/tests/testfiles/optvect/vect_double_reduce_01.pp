...
{ Double-precision reduction autovectorization: sum  s:=s+a[i]  and dot product
  s:=s+a[i]*b[i] are widened to a 2-lane packed accumulator (lane 0 seeded with
  the incoming s, lane 1 zero), accumulated 2-wide, then horizontally summed
  (unpckhpd+addsd) after the loop, with a scalar tail for the residue.
  ...
  |partial sum| well under 2^53) there is no rounding, so every grouping yields
  the identical double value.  The oracle is a DESCENDING (downto) loop, which
