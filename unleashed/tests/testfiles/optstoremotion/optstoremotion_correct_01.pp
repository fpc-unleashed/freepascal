{ %OPT="-O4" }
{ Loop store motion / scalar promotion (-OoSTOREMOTION, on at -O4).  When a loop
  repeatedly loads/stores an invariant-address memory location (v1: a plain
  unmanaged GLOBAL written in the loop), the value is promoted to a register for
  the loop's duration: one load before, register accesses inside, one store back
  after.  Promotion is a pure relocation of where the memory traffic happens --
  it must NEVER change any observable result, for any trip count (including
  zero-trip and early/negative bounds) or any in-loop control-flow path.  Every
  routine below exercises a shape the pass either promotes (for/while/repeat
  accumulation, conditional write, multiple globals, global loop bound) or must
  leave alone (call, aliasing pointer store, break, indexed store, nested loop),
  and asserts the value against an independently computed reference.  Halts with
  the check number on any mismatch. }
program optstoremotion_correct_01;
{$mode objfpc}{$H+}
