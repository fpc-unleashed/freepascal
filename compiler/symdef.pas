...
{$endif}
          ...
          orgdef          : tstoreddef;
          orgdefderef     : tderef;
          { WLG: Witness-based lightweight generics support - declared unconditionally to stabilize ABI }
          wlg_shapeclass          : tshapeclass;         // Shape-class for this specialization
          wlg_identity_hash       : ansistring;          // Identity hash for COMDAT deduplication
          wlg_witness_table_sym   : tsym;                // Witness table symbol reference (tstaticvarsym)
          wlg_witness_table_sym_deref : tderef;          // Deref for witness table symbol
...
{$endif DEBUG_NODE_XML}
       public
          ...
          { WLG: For veneer VMTs - source specialization to copy code addresses from }
          { This is declared here so both objectdef (classes) and recorddef (value types) can use it }
          wlg_veneer_source : tdef;
          wlg_veneer_source_deref : tderef;
...
{$endif DEBUG_NODE_XML}
       public
          ...
          classref_created_in_current_module : boolean;
           ...
           { WLG: wlg_veneer_source is now declared in tabstractrecorddef }
...
{$endif}
           ...
           copied_from : tprocdef;
            { WLG support - uses pointer to avoid circular dependency with symtable.pas }
            witness_parasym: pointer;        // Implicit PWitnessTable parameter (tparavarsym)
            is_shared_generic: boolean;      // WLG shared body flag
            shape_class: tshapeclass;        // Associated Shape-Class
            wlg_veneer_source: tprocdef;     // Direct veneer source for standalone functions and methods
            { WLG Dynamic Stack Frame Generation - Phase 1 }
            has_dynamic_locals: boolean;     // True if this shared generic has local vars of type T
            wlg_stack_base: tsym;            // Internal symbol holding dynamic stack base pointer
            wlg_stack_base_deref: tderef;    // Deref for wlg_stack_base (for PPU serialization)
            { WLG Dynamic Local Init/Final - Phase 4 }
            wlg_dynamic_locals: tfpobjectlist; // List of localvarsym needing Init/Final calls
           constructor create(level:byte;doregister:boolean);virtual;
          constructor ppuload(ppufile:tcompilerppufile);
          destructor  destroy;override;
...
{$IFDEF FPC_HAS_WITNESS_GENERICS}
        { WLG: Load shape-class and identity hash for witness-based generics }
        if df_has_witness in defoptions then
          begin
            { Load shape-class }
            wlg_shapeclass := tshapeclass(ppufile.getbyte);
            { Load identity hash for COMDAT deduplication }
            wlg_identity_hash := ppufile.getansistring;
            { Load witness table symbol reference }
            ppufile.getderef(wlg_witness_table_sym_deref);
          end;
{$ENDIF}
      end;
...
{$endif symansistr}
    ...
    procedure tstoreddef.ppuwrite(ppufile:tcompilerppufile);
      ...
      begin
        ...
        ppufile.putset(tppuset4(defoptions));
...
{$IFDEF FPC_HAS_WITNESS_GENERICS}
        { WLG: Serialize shape-class and identity hash for witness-based generics }
        if df_has_witness in defoptions then
          begin
            { Serialize shape-class }
            ppufile.putbyte(byte(wlg_shapeclass));
            { Serialize identity hash for COMDAT deduplication }
            ppufile.putansistring(wlg_identity_hash);
            { Serialize witness table symbol reference }
            if assigned(wlg_witness_table_sym) then
              ppufile.putderef(wlg_witness_table_sym_deref)
            else
              begin
                { Create a placeholder deref for nil - reset the deref }
                wlg_witness_table_sym_deref.reset;
                ppufile.putderef(wlg_witness_table_sym_deref);
              end;
          end;
{$ENDIF}
...
***************************************************************************}
    ...
    constructor tabstractrecorddef.ppuload(dt:tdeftyp;ppufile:tcompilerppufile);
      ...
      begin
        ...
        {$IFDEF FPC_HAS_WITNESS_GENERICS}
        { WLG Milestone 1: Load wlg_veneer_source reference for cross-unit code sharing }
        if df_shared_generic in defoptions then
          ppufile.getderef(wlg_veneer_source_deref);
        {$ENDIF}
      end;
    ...
    procedure tabstractrecorddef.ppuwrite(ppufile: tcompilerppufile);
      ...
      begin
        ...
        {$IFDEF FPC_HAS_WITNESS_GENERICS}
        { WLG Milestone 1: Write wlg_veneer_source reference for cross-unit code sharing }
        if df_shared_generic in defoptions then
          ppufile.putderef(wlg_veneer_source_deref);
        {$ENDIF}
      end;
...
{$endif}
         ...
         struct := nil;
         structderef.reset;
          { WLG Dynamic Stack Frame Generation - Phase 1 initialization }
          has_dynamic_locals := false;
          wlg_stack_base := nil;
          wlg_stack_base_deref.reset;
          { WLG Dynamic Local Init/Final - Phase 4 initialization }
          wlg_dynamic_locals := nil;
          import_dll:=nil;
         import_name:=nil;
         import_nr:=0;
         inlininginfo:=nil;
...
{$endif MEMDEBUG}
         ...
         freeimplprocdefinfo;
         { WLG Phase 4: Cleanup dynamic locals list }
         if assigned(wlg_dynamic_locals) then
           begin
             wlg_dynamic_locals.free;
             wlg_dynamic_locals := nil;
           end;
...
{$endif EXTDEBUG}
    ...
    procedure tprocdef.buildderef;
      begin
         ...
         { WLG Dynamic Stack Frame Generation - Phase 1 }
         if assigned(wlg_stack_base) then
           wlg_stack_base_deref.build(wlg_stack_base);
      end;
    ...
    procedure tprocdef.deref;
      begin
         ...
         { WLG Dynamic Stack Frame Generation - Phase 1 }
         if wlg_stack_base_deref.dataidx>=0 then
           wlg_stack_base:=tsym(wlg_stack_base_deref.resolve);
      end;
    ...
    function tprocdef.mangledname : TSymStr;
      var
        source_method: tprocdef;
        source_class: tobjectdef;
        source_def: tdef;
        i: longint;
        sym: tsym;
        src_pd: tprocdef;
        src_procsym: tsym;
      begin
        { WLG: If this procedure/function has a direct veneer source, redirect mangled name }
        { This works for both standalone generic functions and methods }
        if assigned(wlg_veneer_source) then
          begin
            result:=wlg_veneer_source.defaultmangledname;
            exit;
          end;
        { Fallback: Check if this is a veneer method via struct.wlg_veneer_source }
        { For classes: VMT entries point to shared code }
        { For records: mangled name redirection ensures call sites link to the first specialization's code }
        source_method:=nil;
        if assigned(self.struct) and (self.struct.typ in [objectdef, recorddef]) then
          begin
            source_def:=tabstractrecorddef(self.struct).wlg_veneer_source;
            if assigned(source_def) and (source_def.typ in [objectdef, recorddef]) then
              begin
                { Get the source record/class and look up the method in its symtable }
                if assigned(tabstractrecorddef(source_def).symtable) and 
                   assigned(self.procsym) and (self.procsym.name<>'') then
                  begin
                    src_procsym:=self.procsym;
                    for i:=0 to tabstractrecorddef(source_def).symtable.SymList.Count-1 do
                      begin
                        sym:=tsym(tabstractrecorddef(source_def).symtable.SymList[i]);
                        if (sym.typ=symconst.procsym) and (upper(sym.realname)=upper(src_procsym.realname)) then
                          begin
                            if tprocsym(sym).ProcdefList.Count>0 then
                              begin
                                src_pd:=tprocdef(tprocsym(sym).ProcdefList[0]);
                                if assigned(src_pd) then
                                  begin
                                    source_method:=src_pd;
                                    break;
                                  end;
                              end;
                          end;
        ...
        if assigned(source_method) then
          begin
            { Get the source method's default mangled name directly }
            { Using defaultmangledname prevents infinite recursion }
            result:=source_method.defaultmangledname;
            exit;
          end;
{$ifdef symansistr}
