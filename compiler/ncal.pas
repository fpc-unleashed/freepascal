...
interface
    ...
    type
       tcallnodeflag = (
         ...
         cnf_asm_inline          { inlined assembler routine: splice its body in place of the call after the normal parameter marshalling }
       );
    ...
    procedure tcallnode.check_inlining;
      ...
      begin
        ...
        { inlined assembler routines cannot go through tree inlining (their
          body reads parameters from ABI locations, which the node-level
          substitution does not set up). Splice the body in codegen instead,
          after the normal parameter marshalling - this needs nostackframe so
          the body has no frame-relative references to a frame that is gone. }
        if forceinline and
           (po_assembler in procdefinition.procoptions) and
           tprocdef(procdefinition).has_inlininginfo then
          begin
            if po_nostackframe in procdefinition.procoptions then
              include(callnodeflags,cnf_asm_inline)
            else
              CGMessagePos2(fileinfo,cg_w_forceinline_not_inlined,tprocdef(procdefinition).procsym.realname,
                'an assembler routine must be "nostackframe" to be inlined');
            exit;
          end;
